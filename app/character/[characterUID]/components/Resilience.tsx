...
const Resilience = () => {
  ...
  const { stats, wounds, resilienceCurrent } = character;

  const [resilience, setResilience] = useState<number>(resilienceCurrent);

  const maxResilience = getDerivedResilienceMax(stats, wounds);
  const resilienceReserves = getResilienceReserves(maxResilience);
  const derivedResilienceMax = getDerivedResilienceMax(stats, wounds);

  const handleResilienceDecrease = () => {
    const decreasedResilience = getDecreasedResilience(
      resilienceReserves,
      resilienceCurrent
    );

    setCharacter({ ...character, resilienceReserves: decreasedResilience });
  };
  ...
  const barColorClass = getBarColorClass(maxResilience, resilience);
  const barState = getBarState(maxResilience, resilience, resilienceReserves);
