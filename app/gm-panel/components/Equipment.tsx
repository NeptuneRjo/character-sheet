...
import { Equipment as EquipmentType, Sheet } from "@/lib/types";
...
const Equipment = ({ equipment, character }: Props) => {
  return (
    <div className="rounded-xl border border-[#5c4a33] bg-[#19130d] px-4 py-3">
      ...
      <div className="mt-3 flex gap-2">
        ...
        >
          {equipment.map((equipment, key) => (
            <option key={key} value={equipment.name}>
              {equipment.name}
            </option>
          ))}
        </select>
      ...
      <ul className="mt-2 space-y-2 text-sm text-[#f0e4cf]">
        {character.equipment.map((equipment, key) => (
          <li
            key={key}
